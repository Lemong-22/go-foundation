...
func (server *Server) handleImportPlan(response http.ResponseWriter, request *http.Request) {
	...
	service, instructorID, err := server.importContext()
	if err != nil {
		writeImportError(response, err)
		return
	}
...
func (server *Server) importContext() (core.ImportService, string, error) {
	if server.runner.imports == nil {
		return nil, "", ErrMissingImportService
	}
	instructorID := server.runner.instructorID()
	if instructorID == "" {
		return nil, "", domain.NewValidationError("instructor_id", "is required")
	}

	return server.runner.imports, instructorID, nil
}
...
func importZipTempfile(request *http.Request) (string, func(), error) {
	...
	temp, err := os.CreateTemp("", "course-console-import-*.zip")
	if err != nil {
		return "", func() {}, err
	}
...
func writeImportError(response http.ResponseWriter, err error) {
	status := importStatusForError(err)
	message := err.Error()
	if status == http.StatusInternalServerError {
		message = "internal error"
	}
	writeJSON(response, status, importErrorResponse{Error: message})
}

func importStatusForError(err error) int {
	switch {
